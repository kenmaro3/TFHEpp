...
double my_mult_function(double x, double y){
  return x*y;
}

double my_batch_normalization_function(double x, double gamma, double mean, double var, double beta){
  // from keras
...
template <class P>
vector<TLWE<P>> encrypt_vector(const vector<double> xs, const array<typename P::T, P::n> sk, Encoder &encoder){
  vector<TLWE<P>> res;
  for(int i=0; i<xs.size(); i++){
    res.push_back(TFHEpp::tlweSymEncodeEncrypt<P>(xs[i], P::alpha, sk, encoder));
  }
...
template <class P>
vector<TLWE<P>> encrypt_vector_omp(const vector<double> xs, const array<typename P::T, P::n> sk, Encoder &encoder){
...
template <class P>
vector<double> decrypt_vector_omp(const vector<TLWE<P>> cs, const array<typename P::T, P::n> sk, Encoder &encoder, bool is_print=true){
...
template <class P>
double decrypt(const TLWE<P> cs, const array<typename P::T, P::n> sk, Encoder &encoder, bool is_print=true){
...
template <class p>
vector<TLWE<p>> multd(const vector<TLWE<p>> cs, const vector<double> ds, const GateKey &gk, Encoder &encoder_domain, Encoder &encoder_target, double expansion=1.){
  assert(cs.size() == ds.size());
  vector<TLWE<p>> res(cs.size());

  for(int i=0; i<cs.size(); i++){
    //encoder_domain.print();
    TFHEpp::ProgrammableBootstrapping(res[i], cs[i], gk, encoder_domain, encoder_target, my_mult_function, ds[i]); 
  }
...
template <class p>
vector<TLWE<p>> multd(const vector<TLWE<p>> cs, const vector<double> ds, const GateKey &gk, Encoder &encoder, double expansion=1.){
...
template <class p>
vector<TLWE<p>> relu(const vector<TLWE<p>> cs, const GateKey &gk, Encoder &encoder, double expansion=1.){
  Encoder encoder_domain = Encoder::copy(encoder);
  Encoder encoder_target = Encoder::copy(encoder);
  vector<TLWE<p>> res(cs.size());
  ...
  for(int i=0; i<cs.size(); i++){
    //encoder_domain.print();
    TFHEpp::ProgrammableBootstrapping(res[i], cs[i], gk, encoder_domain, encoder_target, my_relu_function); 
  }
...
template <class p>
vector<TLWE<p>> relu_omp(const vector<TLWE<p>> cs, const GateKey &gk, Encoder &encoder, double expansion=1.){
  Encoder encoder_domain = Encoder::copy(encoder);
  Encoder encoder_target = Encoder::copy(encoder);
  vector<TLWE<p>> res(cs.size());
...
template <class p>
vector<TLWE<p>> sigmoid(const vector<TLWE<p>> cs, const GateKey &gk, Encoder &encoder, double expansion=1.){
  Encoder encoder_domain = Encoder::copy(encoder);
  ...
  for(int i=0; i<cs.size(); i++){
    //encoder_domain.print();
    TFHEpp::ProgrammableBootstrapping(res[i], cs[i], gk, encoder_domain, encoder_target, my_sigmoid_function); 
  }
...
template <class p>
vector<TLWE<p>> sigmoid_omp(const vector<TLWE<p>> cs, const GateKey &gk, Encoder &encoder, double expansion=1.){
  Encoder encoder_domain = Encoder::copy(encoder);
  Encoder encoder_target = Encoder(-2, 2, encoder.bp);
  vector<TLWE<p>> res(cs.size());
...
template <class P>
TLWE<P> inner(const vector<TLWE<P>> cs, const vector<double> ds, const GateKey &gk, Encoder &encoder_domain, Encoder &encoder_target, double expansion=1.0){
  vector<TLWE<P>> tmp = multd<P>(cs, ds, gk, encoder_domain, encoder_target, expansion);
  TLWE<P> res = tmp[0];
  for(int i=1; i<cs.size(); i++){
    TFHEpp::HomADDFixedEncoder(res, res, tmp[i], encoder_target, encoder_target);
  }
  return res;
}

template <class P>
TLWE<P> inner(const vector<TLWE<P>> cs, const vector<double> ds, const GateKey &gk, Encoder &encoder, double expansion=1.0){
...
template <class P>
vector<TLWE<P>> wtx(const vector<TLWE<P>> cs, const vector<vector<double>> w, const GateKey &gk, Encoder &encoder, double expansion){
...
template <class P>
vector<TLWE<P>> wtx_omp(const vector<TLWE<P>> cs, const vector<vector<double>> w, const GateKey &gk, Encoder &encoder, double expansion){
  assert(cs.size() == w[0].size());
...
template <class P>
vector<TLWE<P>> btx(const vector<TLWE<P>> cs, const vector<double> b, Encoder &encoder){
...
template <class P>
vector<TLWE<P>> btx_omp(const vector<TLWE<P>> cs, const vector<double> b, Encoder &encoder){
  assert(cs.size() == b.size());
...
template <class P>
vector<TLWE<P>> linear(const vector<TLWE<P>> cs, const vector<vector<double>> w, const vector<double> b, const GateKey &gk, Encoder &encoder){
...
template <class P>
vector<TLWE<P>> linear_omp(const vector<TLWE<P>> cs, const vector<vector<double>> w, const vector<double> b, const GateKey &gk, Encoder &encoder){
...
template <class P>
vector<TLWE<P>> nonlinear(const vector<TLWE<P>> cs, const GateKey gk, Encoder &encoder, string nonlinear_type){
...
template <class P>
vector<TLWE<P>> nonlinear_omp(const vector<TLWE<P>> cs, const GateKey gk, Encoder &encoder, string nonlinear_type){
  if(nonlinear_type=="sigmoid"){
...
vector<double> linear(const vector<double> x, const vector<vector<double>> w, vector<double> b){
...
void test_omp(){
  ...
  printf("\nencryption=============================================================\n");
  vector<TLWE<lvl0param>> c1 = encrypt_vector_omp<lvl0param>(x1, sk->key.lvl0, encoder);
  ...
  for(int i=0; i<3; i++){
    ...
    printf("\ncipher=============================================================\n");
    vector<TLWE<lvl0param>> a1 = linear_omp<lvl0param>(c1, w, b, *gk.get(), encoder);
    if(is_decrypt)
      printf("\nlineared \n");
      decrypt_vector<lvl0param>(a1, sk->key.lvl0, encoder);
    //vector<TLWE<lvl0param>> y1 = nonlinear<lvl0param>(a1, *gk.get(), encoder, nl_type1);
    printf("\nnon-lineared\n");
    vector<TLWE<lvl0param>> y1 = sigmoid_omp<lvl0param>(a1, *gk.get(), encoder);
    if(is_decrypt)
      decrypt_vector<lvl0param>(y1, sk->key.lvl0, encoder);
    ...
    printf("\ncipher=============================================================\n");
    vector<TLWE<lvl0param>> a2 = linear_omp<lvl0param>(y1, w2, b2, *gk.get(), encoder);
    printf("\nlineared\n");
    if(is_decrypt)
      decrypt_vector<lvl0param>(a2, sk->key.lvl0, encoder);
    //vector<TLWE<lvl0param>> y2 = nonlinear<lvl0param>(a2, *gk.get(), encoder, nl_type2);
    vector<TLWE<lvl0param>> y2 = sigmoid_omp<lvl0param>(a2, *gk.get(), encoder);
    printf("\nnon-lineared\n");
    if(is_decrypt)
      decrypt_vector<lvl0param>(y2, sk->key.lvl0, encoder);
